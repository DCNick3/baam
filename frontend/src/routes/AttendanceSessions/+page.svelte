...
<div class="flex flex-row justify-center w-full text-left">
    <div class="flex flex-col w-4/5 mt-3">
        {#if selection.length>0}
            <div class="grid grid-flow-row border-t-[1px] grid-cols-[5%_95%] border-gray-300 text-slate-800" > 
                <div class="p-3"> </div>
                <div class="flex flex-row">
                    <div class="p-3"> <Button type="Primary"> Export {selection.length} sessions</Button> </div>
                    <div class="p-3 pl-0"> <Button type="Primary"> Delete {selection.length} sessions</Button> </div>
                </div>
            </div>
        {:else}
        <div class="grid grid-flow-row border-t-[1px] grid-cols-[5%_95%] border-gray-300 text-slate-800" > 
            <div class="p-3"> </div>
            <div class="flex flex-row">
                <div class="p-3"> <Button disabled={true} type="Secondary"> Export {selection.length} sessions</Button> </div>
                <div class="p-3 pl-0"> <Button disabled={true} type="Secondary"> Delete {selection.length} sessions</Button> </div>
            </div>
        </div>
        {/if}
        {#each sessions as session}
            <div class="grid grid-flow-row grid-cols-[5%_15%_72%_8%] even:border-t-[1px] even:border-b-[1px] last:border-b-[1px] border-gray-300 text-slate-800">
                <div class="mt-[14px]">
                    <label class="lbl-checkbox p-[10px] pl-[15px] pr-[15px] rounded-full hover:odd:bg-neutral-50">
                        <input aria-label="Select session {session.title}" type="checkbox" bind:group={selection} value={session.id} />
                    </label>
                </div>
